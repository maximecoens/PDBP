...
from models.helper import _keypoints_and_edges_for_display, to_gif, draw_prediction_on_image, \
    ...
    torso_visible
import os
from exercises.bovenhandseCurl import compare
...
def compare2(correctFrame, inputFrame):
  ...
  result = 0
  for coord in range(len(correctFrame)):
...
def predict_movenet_for_video(video_path, exercise, states):
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            # Compare frame
            # TODO: states lijst maken met alle correcte frames en dan met indexen werken
            similarity_score = compare2(keypoints_with_scores, states[current_state])
...
def predict_movenet_for_webcam(video_path, exercise):
    model_name = "movenet_lightning"
    interpreter = tf.lite.Interpreter(model_path="models/lite-model_movenet_singlepose_lightning_3.tflite")
    input_size = 192 
